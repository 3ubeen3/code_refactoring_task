...
class BookingRepository extends BaseRepository
{
    ...
    public function storeJobEmail($data)
    {
        ...
        $name = $user->name;
        $subject = 'Some testing Subject added By Mubeen : #' . $job->id;
        $send_data = [
            'user' => $user,
            'job'  => $job
        ];
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $jobs = Job::getJobs($user_id, $job_type, 'pending', $languages, $gender, $translator_level);
        ...
        return TeHelper::convertJobIdsInObjs($filtered_jobs);
    }
    ...
    {
        $suitable_translators = User::where('user_type', 2)
            ->where('status', 1)
            ->where('id', '!=', $exclude_user_id)
            ->where('not_get_emergency', '!=', 'yes')
            ->where(function ($query) {
                $query->where('last_push_sent', '<', Carbon::now()->subMinutes(5))
                    ->orWhereNull('last_push_sent');
            })
            ->get();
        ...
        $logger->addInfo('Push send for job ' . $job->id, [$suitable_translators, $msg_text, $data]);

        $this->sendPushNotificationToSpecificUsers($suitable_translators, $job->id, $data, $msg_text, false);
    }
